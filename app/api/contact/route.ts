...
const FROM = process.env.EMAIL_FROM ?? 'GetHireToday Contact <hello@gethiretoday.com>';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeSubject = (subject && String(subject).trim()) || 'New GetHireToday contact form message';
    ...
    const { error } = await resend.emails.send({
      from: FROM,
      to: TO,
      subject: `[GetHireToday] ${safeSubject}`,
      replyTo: email,
      html,
    });
